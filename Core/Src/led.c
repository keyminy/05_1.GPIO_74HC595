...
void second_led_74hc595_demo(void);
...
//		GPIOB->ODR &= ~GPIO_PIN_7; // latch핀을 pull-down
//		GPIOB->ODR |= GPIO_PIN_7; // // latch핀을 pull-up
//		HAL_Delay(1000);
//	}
	#endif
...
void second_led_74hc595_demo(void){
	...
	if(TIM2_74HC595_counter >= 200){
		TIM2_74HC595_counter = 0;
		HAL_SPI_Transmit(&hspi2, &led_buff[led_status], 1, 1);
		GPIOA->ODR &= ~GPIO_PIN_11; // latch핀을 pull-down
		GPIOA->ODR |= GPIO_PIN_11; // // latch핀을 pull-up
		led_status++;
		led_status%=2;
	}
//	for (int i=0; i< 4; i++)
